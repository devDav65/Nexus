...
export default function ChatHeader({ conversation, otherMember, currentUserId, onAudioCall, onVideoCall }: ChatHeaderProps) {
  ...
  // ── 1. DÉCLARATION DES VARIABLES AVANT LE USEEFFECT ──────────────────
  ...
  // Charger l'état de blocage au montage
  useEffect(() => {
    if (!isDM || !profile?.id) return
    supabase
        .from("blocked_users")
        .select("id")
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-medium">
                            {isBlocked ? "Débloquer" : "Bloquer"} {displayName}
                          </p>
                          {!isBlocked && (
                              <p className="text-xs opacity-60">
                                Il ne pourra plus t'envoyer de messages
                              </p>
                          )}
                        </div>
                      </button>

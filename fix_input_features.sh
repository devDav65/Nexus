...
export default function MessageInput({
  ...
  const startRecording = async () => {
    try {
      ...
      setRecording(true)
      setRecordingTime(0)
      timerRef.current = setInterval(() => setRecordingTime(p => { if (p >= 120) { stopRecording(); return p } return p + 1 }), 1000)
  ...
  const uploadAudio = async (blob: Blob) => {
    ...
    const { error } = await supabase.storage.from("attachments").upload(path, blob, { contentType: "audio/webm", upsert: true })
    if (!error) {
      const { data: { publicUrl } } = supabase.storage.from("attachments").getPublicUrl(path)
      await supabase.from("messages").insert({
        conversation_id: conversationId, sender_id: currentUserId,
        content: "🎤 Message vocal", type: "audio",
        metadata: { audio_url: publicUrl, duration: recordingTime },
      })
    }
    ...
    <div className="shrink-0 bg-background">
      ...
      {showEmoji && (
        <div className="border-t border-border px-3 py-2 bg-background">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setShowEmoji(false)} className="text-muted-foreground hover:text-foreground">
              <X className="w-4 h-4" />
            </button>
          </div>
      ...
      {showAttach && (
        <div className="border-t border-border px-4 py-3 bg-background flex gap-3">
          <button
            onClick={() => { fileInputRef.current!.accept = "image/*"; fileInputRef.current!.click(); setShowAttach(false) }}
      ...
      {recording && (
        <div className="flex items-center gap-3 bg-red-500/10 border-t border-red-500/20 px-4 py-2">
          ...
          <span className="text-sm text-red-500 font-medium flex-1">Enregistrement… {formatTime(recordingTime)}</span>
      ...
      <div className="px-3 py-2 border-t border-border">
        <div className="flex items-end gap-2 bg-muted/50 rounded-2xl px-3 py-2 border border-border focus-within:border-primary/50 transition-colors">
          ...
          <button onClick={() => { setShowAttach(p => !p); setShowEmoji(false) }}
            ...
            className={cn("w-8 h-8 rounded-full flex items-center justify-center shrink-0 mb-0.5 transition-colors",
          ...
          {canSend ? (
            ...
            <button onClick={recording ? stopRecording : startRecording}
              ...
              className={cn("w-8 h-8 rounded-full shrink-0 mb-0.5 flex items-center justify-center transition-colors",
                recording ? "bg-red-500 text-white" : "text-muted-foreground hover:text-primary")}>
              {uploadingAudio ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : recording ? <Square className="w-3.5 h-3.5" /> : <Mic className="w-3.5 h-3.5" />}
            </button>
          )}
        </div>
      </div>
